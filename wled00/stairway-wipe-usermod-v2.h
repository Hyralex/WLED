...
//#define STAIRCASE_WIPE_OFF
  ...
  if (userVar0 > 0)
  {
    ...
    void readFromJsonState(JsonObject& root)
    {
      ...
      fadeOutDelay = root["fadeOutDelay"] | fadeOutDelay;
    ...
    void addToConfig(JsonObject& root)
    {
      ...
      top["user0"] = userVar0; //save this var persistently whenever settings are saved
    }

    void readFromConfig(JsonObject& root)
    {
      JsonObject top = root["top"];
      userVar0 = top["great"] | 0; //The value right of the pipe "|" is the default value in case your setting was not present in cfg.json (e.g. first boot)
    }
    ...
    {
    #ifdef STAIRCASE_WIPE_OFF
    transitionDelayTemp = 1; //turn off immediately after wipe completed
    #else
    transitionDelayTemp = fadeOutDelay; //fade out slowly
    #endif
    bri = 0;
    colorUpdated(NOTIFIER_CALL_MODE_NOTIFICATION);
    wipeState = 0;
